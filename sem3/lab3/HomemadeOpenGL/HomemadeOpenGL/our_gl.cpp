...
// Убираем глобальные определения матриц
// Matrix ModelView;
// Matrix Viewport;
// Matrix Projection;

// на всякий случай пока оставил старые функции тут
...
void triangle(Vec3i* pts, IShader& shader, TGAImage& image, TGAImage& zbuffer) {
  ...
  for (P.x = bboxmin.x; P.x <= bboxmax.x; P.x++) {
    for (P.y = bboxmin.y; P.y <= bboxmax.y; P.y++) {
      ...
      P.z = std::max(0, std::min(255, int(pts[0].z * c.x + pts[1].z * c.y +
                                          pts[2].z * c.z + .5)));
